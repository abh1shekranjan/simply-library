...
function addBook(req, res) {
    ...
    Promise.resolve()
        ...
        .then(() => userCollection.findOne({ title: title, authors: authors, publisher: publisher }))
        .then((result) => {
            ...
            if (result.insertedCount === 1 || (result.result ? result.result.ok : false)) {
                console.log("book added successfully");
                return res.status(200).json({ "message": "New book is added in the booklist" });
            } else {
...
function updateRecord(record, userCollection) {
    ...
    if (rackFlag === -1)
        rack.push(bookCopyId);

    return userCollection.updateOne({ "_id": record._id }, { $set: { "rack": rack, "bookCopy": bookCopy } });
}
